...
function LabRows({ labs, machines, onOpen }: { labs: ApiLab[]; machines: ApiMachine[]; onOpen: (lab: ApiLab) => void }) {
  return <div className="divide-y divide-ink/10">{labs.map((lab) => {
    ...
    return <button key={lab.id} type="button" onClick={() => onOpen(lab)} className="block w-full p-4 text-left transition hover:bg-cloud/60 focus:bg-cloud/60 focus:outline-none"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-base font-black text-ink">{lab.name}</h3><span className={clsx("rounded-full px-2.5 py-1 text-xs font-bold", locked ? "bg-ink/8 text-ink/45" : running ? "bg-mint/25 text-fern" : "bg-sun/20 text-ink/70")}>{locked ? "Locked" : running ? "Running" : "Ready"}</span></div><p className="mt-1 text-sm leading-6 text-ink/58">{lab.description}</p><p className="mt-2 text-xs font-semibold text-ink/48">{lab.level} · {lab.runtime} · {machineNames}</p></div><span className="shrink-0 rounded-md border border-fern/25 px-3 py-2 text-sm font-bold text-fern">Open lab</span></div><p className="mt-3 text-xs text-ink/52"><span className="font-bold text-fern">Next:</span> {lab.next_step}</p></button>;
  })}</div>;
}
...
function LabWorkspace({ apiUrl, lab, machines, busy, reportLog, onClose, onStart, onStop, onDownloadVpn, onReport, onAnswersSaved, onSubmitted }: { apiUrl: string; lab: ApiLab; machines: ApiMachine[]; busy: boolean; reportLog: string; onClose: () => void; onStart: (lab: ApiLab) => Promise<void>; onStop: (lab: ApiLab) => Promise<void>; onDownloadVpn: (lab: ApiLab) => Promise<void>; onReport: (lab: ApiLab) => Promise<void>; onAnswersSaved: (labId: string, questions: ApiLab["questions"]) => void; onSubmitted: (labId: string, score: number, maxScore: number) => void }) {
  ...
  const assignedMachines = lab.machine_ids.map((id) => machines.find((machine) => machine.id === id)).filter((machine): machine is ApiMachine => Boolean(machine));

  useEffect(() => {
    setAnswers(Object.fromEntries(lab.questions.map((item) => [item.id, item.answer])));
  }, [lab.id, lab.questions]);
  useEffect(() => {
    if (lab.status !== "running") { setAttachments([]); return; }
    getLabAttachments(apiUrl, lab.id).then((result) => setAttachments(result.attachments)).catch(() => setAttachments([]));
  ...
  async function getAttachment(item: LabAttachment) {
    try { const result = await downloadLabAttachment(apiUrl, item); const url = URL.createObjectURL(result.blob); const link = document.createElement("a"); link.href = url; link.download = result.filename; link.click(); URL.revokeObjectURL(url); }
    catch (reason) { setMessage(reason instanceof Error ? reason.message : "Unable to download attachment."); }
  }

  return <div className="fixed inset-0 z-50 overflow-y-auto bg-cloud"><header className="sticky top-0 z-10 border-b border-ink/10 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6"><button type="button" onClick={onClose} className="inline-flex min-h-10 items-center gap-2 text-sm font-bold text-ink/65 hover:text-fern"><ArrowLeft size={18} />Back to labs</button><div className="flex flex-wrap gap-2">{lab.status === "running" ? <><Action icon={Download} onClick={() => onDownloadVpn(lab)} disabled={busy}>VPN config</Action><Action icon={ClipboardCheck} onClick={() => onReport(lab)} disabled={busy}>Attack report</Action><button type="button" onClick={() => onStop(lab)} disabled={busy} className="inline-flex min-h-10 items-center gap-2 rounded-md border border-clay/30 px-3 text-sm font-bold text-clay"><X size={16} />Stop lab</button></> : <Action icon={busy ? LoaderCircle : Play} spinning={busy} onClick={startWorkspace} disabled={busy || lab.status === "locked"}>{lab.status === "locked" ? "Locked" : busy ? "Starting" : "Start lab"}</Action>}</div></div></header><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6"><div className="rounded-xl bg-ink p-6 text-white shadow-lg sm:p-8"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold uppercase">{lab.level}</span><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold uppercase">{lab.runtime}</span><span className={clsx("rounded-full px-3 py-1 text-xs font-bold uppercase", lab.status === "running" ? "bg-mint text-fern" : "bg-white/10")}>{lab.status}</span></div><h1 className="mt-5 text-3xl font-black sm:text-4xl">{lab.name}</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-white/70 sm:text-base">{lab.description}</p><p className="mt-5 text-sm font-bold text-mint">{lab.next_step}</p></div>{message ? <p className="mt-4 rounded-lg border border-fern/25 bg-mint/20 px-4 py-3 text-sm font-bold text-fern">{message}</p> : null}<div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]"><div className="space-y-6"><Panel eyebrow="Lab questions" title="Record your findings"><div className="space-y-5 p-4 sm:p-6">{lab.questions.length ? lab.questions.map((question, index) => <label key={question.id} className="block"><span className="flex gap-3 text-sm font-black text-ink"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-mint/30 text-xs text-fern">{index + 1}</span>{question.prompt}</span><textarea value={answers[question.id] ?? ""} onChange={(event) => setAnswers((items) => ({ ...items, [question.id]: event.target.value }))} className="mt-3 min-h-28 w-full resize-y rounded-lg border border-ink/15 p-3 text-sm leading-6 outline-none focus:border-fern" placeholder="Enter your answer, evidence, or flag…" maxLength={4000} /></label>) : <p className="text-sm text-ink/50">This lab has no questions yet.</p>} {lab.questions.length ? <div className="flex justify-end"><Action icon={saving ? LoaderCircle : Save} spinning={saving} onClick={saveAnswers} disabled={saving}>{saving ? "Saving" : "Save answers"}</Action></div> : null}</div></Panel>{reportLog ? <Panel eyebrow="Telemetry report" title="Attack chain reconstruction"><pre className="max-h-96 overflow-auto whitespace-pre-wrap bg-ink p-4 text-xs leading-5 text-white">{reportLog}</pre></Panel> : null}<Panel eyebrow="Machine layout" title="Targets in this lab"><div className="grid gap-px bg-ink/10 sm:grid-cols-2">{assignedMachines.map((machine) => <div key={machine.id} className="bg-white p-5"><div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center rounded-lg bg-mint/20 text-fern"><ServerCog size={19} /></span><span className="rounded bg-cloud px-2 py-1 text-xs font-bold text-ink/50">{machine.os_type}</span></div><h3 className="mt-4 font-black text-ink">{machine.name}</h3><p className="mt-2 text-sm leading-6 text-ink/55">{machine.description}</p><p className="mt-3 truncate text-xs font-semibold text-fern">{machine.imageUrl}</p></div>)}</div></Panel></div><aside className="space-y-6"><Panel eyebrow="Connection" title="Lab access"><div className="p-4"><div className={clsx("rounded-lg p-4", lab.status === "running" ? "bg-mint/20" : "bg-cloud")}><ShieldCheck className={lab.status === "running" ? "text-fern" : "text-ink/35"} /><p className="mt-3 text-sm font-black text-ink">{lab.status === "running" ? "Environment running" : "Environment offline"}</p><p className="mt-1 text-xs leading-5 text-ink/55">{lab.status === "running" ? "Download the WireGuard configuration and connect to the isolated lab network." : "Start this lab to unlock VPN access and attachments."}</p></div>{lab.status === "running" ? <button type="button" onClick={() => onDownloadVpn(lab)} className="mt-3 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md bg-canopy px-3 text-sm font-bold text-white"><Download size={16} />Download VPN config</button> : null}</div></Panel><Panel eyebrow="Resources" title="Attachments"><div className="divide-y divide-ink/10">{lab.status !== "running" ? <p className="p-4 text-sm text-ink/50">Start the lab to unlock downloads.</p> : attachments.length ? attachments.map((item) => <button type="button" key={item.machine_id + item.download_url} onClick={() => getAttachment(item)} className="flex w-full items-center gap-3 p-4 text-left hover:bg-cloud"><span className="grid h-9 w-9 place-items-center rounded bg-cloud text-fern"><FileText size={17} /></span><span className="min-w-0"><span className="block truncate text-sm font-black text-ink">{item.filename}</span><span className="block truncate text-xs text-ink/45">{item.machine_name}</span></span><Download size={16} className="ml-auto shrink-0 text-fern" /></button>) : <p className="p-4 text-sm text-ink/50">No attachments for this lab.</p>}</div></Panel></aside></div></main></div>;
}
...
function Builder({
  ...
  async function start(scenario: ApiScenario) {
    ...
    try {
      ...
      onUpdated({ ...scenario, status: "running", running_session_id: result.id });
    } catch (reason) {
  ...
  async function downloadVpn(scenario: ApiScenario) {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Unable to download the VPN config.");
    } finally {
      setBusyScenario(null);
    }
  }
...
export default function StudentPortal({ apiUrl, view }: { apiUrl: string; view: string }) {
  ...
  async function report(lab: ApiLab) { setBusyLab(lab.id); setNotice(""); try { const sessions = await listLabSessions(apiUrl, lab.id); const latest = [...sessions].sort((left, right) => Date.parse(right.started_at) - Date.parse(left.started_at))[0]; if (!latest) { setNotice("No sessions are available for " + lab.name + "."); return; } const [result, pdfDocument] = await Promise.all([getAttackReport(apiUrl, latest.id), downloadAttackReport(apiUrl, latest.id)]); setReportLog(formatAttackReport(result)); const url = URL.createObjectURL(pdfDocument.blob); const link = document.createElement("a"); link.href = url; link.download = pdfDocument.filename; link.click(); URL.revokeObjectURL(url); setNotice("Downloaded " + pdfDocument.filename + "."); } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Unable to generate the attack-chain PDF report."); } finally { setBusyLab(null); } }
  ...
  const labs = <Panel eyebrow="Assigned labs" title="Continue learning"><LabRows labs={data.assignments} machines={data.machines} onOpen={(lab) => { setOpenLabId(lab.id); setReportLog(""); }} /></Panel>;
