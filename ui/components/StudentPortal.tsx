...
import { Activity, Check, Clock3, Cpu, Download, Edit3, Layers3, Play, Plus, ServerCog, Trash2, X } from "lucide-react";
import { deleteScenario, getStudentDashboard, saveScenario, startLab, stopLab, updateScenario, type ApiLab, type ApiMachine, type ApiScenario, type StudentDashboard } from "@/lib/api";
...
function LabRows({ labs, machines, busyLab, onDownload, onStart, onStop }: { labs: ApiLab[]; machines: ApiMachine[]; busyLab: string | null; onDownload: (lab: ApiLab) => void; onStart: (lab: ApiLab) => void; onStop: (lab: ApiLab) => void }) {
  return <div className="divide-y divide-ink/10">{labs.map((lab) => {
    ...
    return <article key={lab.id} className="p-4"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-base font-black text-ink">{lab.name}</h3><span className={clsx("rounded-full px-2.5 py-1 text-xs font-bold", locked ? "bg-ink/8 text-ink/45" : running ? "bg-mint/25 text-fern" : "bg-sun/20 text-ink/70")}>{locked ? "Locked" : running ? "Running" : "Ready"}</span></div><p className="mt-1 text-sm leading-6 text-ink/58">{lab.description}</p><p className="mt-2 text-xs font-semibold text-ink/48">{lab.level} - {lab.runtime} - {machineNames}</p></div><div className="flex flex-wrap gap-2">{running ? <><Action icon={Download} onClick={() => onDownload(lab)} disabled={busyLab === lab.id}>{busyLab === lab.id ? "Preparing" : "Download VPN"}</Action><button type="button" onClick={() => onStop(lab)} disabled={busyLab === lab.id} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-clay/25 px-3 text-sm font-bold text-clay hover:bg-clay/10 disabled:cursor-not-allowed disabled:opacity-50"><X size={16} />Stop</button></> : <Action icon={Play} onClick={() => onStart(lab)} disabled={locked || busyLab === lab.id}>{locked ? "Locked" : busyLab === lab.id ? "Starting" : "Start"}</Action>}</div></div>{lab.tasks.length ? <div className="mt-4 rounded-md border border-ink/10 bg-cloud/60 p-3"><p className="text-xs font-bold uppercase text-ink/45">Flags and questions</p><ul className="mt-2 grid gap-2 text-sm text-ink/68">{lab.tasks.map((task) => <li key={task} className="flex gap-2"><span className="mt-1 grid h-4 w-4 shrink-0 place-items-center rounded-full border border-fern/35 text-transparent">.</span><span>{task}</span></li>)}</ul></div> : null}<p className="mt-3 text-xs text-ink/52"><span className="font-bold text-fern">Next:</span> {lab.next_step}</p></article>;
  })}</div>;
}
...
export default function StudentPortal({ apiUrl, view }: { apiUrl: string; view: string }) {
  const [data, setData] = useState<StudentDashboard | null>(null); const [error, setError] = useState(""); const [busyLab, setBusyLab] = useState<string | null>(null); const [notice, setNotice] = useState(""); const [scenarios, setScenarios] = useState<ApiScenario[]>([]);
  ...
  async function download(lab: ApiLab) { setBusyLab(lab.id); setNotice(""); try { const result = await startLab(apiUrl, lab.id); downloadConfig(result.wireguard_filename, result.wireguard_config); setLabStatus(lab.id, "running", "Download the VPN config and connect with WireGuard"); setNotice("Downloaded " + result.wireguard_filename + "."); } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Unable to download the VPN config."); } finally { setBusyLab(null); } }
  ...
  const labs = <Panel eyebrow="Assigned labs" title="Continue learning"><LabRows labs={data.assignments} machines={data.machines} busyLab={busyLab} onDownload={download} onStart={start} onStop={stop} /></Panel>;
