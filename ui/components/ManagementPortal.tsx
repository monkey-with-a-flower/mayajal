...
import { Activity, Check, ClipboardCheck, Download, Edit3, Layers3, LoaderCircle, Play, Plus, Save, ServerCog, Square, Trash2, Users, X } from "lucide-react";
import {
  changeSetting,
  changeUserRole,
  completeReview,
...
function Button({ children, icon: Icon, onClick, disabled, spinning }: { children: React.ReactNode; icon: typeof Plus; onClick?: () => void; disabled?: boolean; spinning?: boolean }) {
...
function formatAttackReport(report: AttackReport) {
  ...
  report.attack_chain.forEach((phase, index) => {
    ...
    phase.evidence.slice(0, 3).forEach((item) => lines.push("   evidence: " + JSON.stringify(item)));
  });
  return lines.join("\n");
}

function latestSession(sessions: ApiLabSession[]) {
  return [...sessions].sort((left, right) => Date.parse(right.started_at) - Date.parse(left.started_at))[0];
}
...
function LabForm({
  ...
  function toggleStudent(id: string) {
    onChange({ ...value, student_ids: value.student_ids.includes(id) ? value.student_ids.filter((item) => item !== id) : [...value.student_ids, id] });
  }
  ...
  return <form onSubmit={onSubmit} className="space-y-5 p-4">
    ...
    {students.length || groups.length ? <div className="grid gap-4 lg:grid-cols-2">
      <div>
        ...
        <div className="mt-3 grid gap-2">{groups.length ? groups.map((group) => <label key={group.id} className={clsx("flex min-h-11 items-center justify-between gap-3 rounded-md border px-3 text-sm font-bold", value.group_ids.includes(group.id) ? "border-fern bg-mint/10 text-fern" : "border-ink/10 text-ink/65")}><span>{group.name}<span className="ml-2 text-xs font-semibold text-ink/45">{group.student_count} students</span></span><input type="checkbox" checked={value.group_ids.includes(group.id)} onChange={() => toggleGroup(group.id)} className="h-4 w-4 accent-[#2f6f5f]" /></label>) : <p className="rounded-md bg-cloud p-3 text-sm font-semibold text-ink/54">Create a student group first.</p>}</div>
      </div>
      <div>
        <p className="text-sm font-black text-ink">Assign individual students</p>
        <div className="mt-3 grid gap-2">{students.map((student) => <label key={student.id} className={clsx("flex min-h-11 items-center justify-between gap-3 rounded-md border px-3 text-sm font-bold", value.student_ids.includes(student.id) ? "border-fern bg-mint/10 text-fern" : "border-ink/10 text-ink/65")}><span>{student.name}<span className="ml-2 text-xs font-semibold text-ink/45">{student.cohort}</span></span><input type="checkbox" checked={value.student_ids.includes(student.id)} onChange={() => toggleStudent(student.id)} className="h-4 w-4 accent-[#2f6f5f]" /></label>)}</div>
      </div>
    </div> : null}
...
function LabList({
  ...
  onReport?: (lab: ApiLab) => Promise<void>;
}) {
  ...
  async function report(lab: ApiLab) {
    if (!onReport) return;
    setBusyId(lab.id);
    try {
      await onReport(lab);
  ...
  return <div className="divide-y divide-ink/10">{labs.length ? labs.map((lab) => {
    ...
    return <div key={lab.id} className="p-4">
      {isEditing ? <div className="space-y-3">
        <LabForm machines={machines} students={students} groups={groups} value={draft} onChange={setDraft} submitLabel="Save lab" saving={disabled} onSubmit={(event) => { event.preventDefault(); save(lab); }} />
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs font-semibold text-ink/42">{lab.level} - {lab.runtime} - {lab.owner} - {lab.machine_ids.length} machines - {lab.tasks.length} questions - {lab.assigned_count ?? 0} assigned</p>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          {onStart || onStop ? lab.status === "running" ? <><Button icon={disabled ? LoaderCircle : Download} spinning={disabled} onClick={() => downloadVpn(lab)} disabled={disabled || !onDownloadVpn}>{disabled ? "Preparing" : "Get VPN"}</Button><Button icon={disabled ? LoaderCircle : ClipboardCheck} spinning={disabled} onClick={() => report(lab)} disabled={disabled || !onReport}>{disabled ? "Loading" : "Report"}</Button><button type="button" onClick={() => stop(lab)} disabled={disabled} className="inline-flex min-h-9 items-center justify-center gap-2 rounded-md border border-clay/25 px-3 text-xs font-bold text-clay hover:bg-clay/10 disabled:opacity-50">{disabled ? <LoaderCircle size={15} className="animate-spin" /> : <Square size={15} />}Stop</button></> : <Button icon={disabled ? LoaderCircle : Play} spinning={disabled} onClick={() => start(lab)} disabled={disabled || lab.status === "locked"}>{disabled ? "Starting" : "Start"}</Button> : null}
          {editable ? <>
...
function TeacherPortal({ apiUrl, view }: { apiUrl: string; view: string }) {
  ...
  function resetGroupForm() { setEditingGroupId(null); setGroupName(""); setGroupStudents([]); }
  async function saveGroup(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!groupName.trim()) return; try { const payload = { name: groupName.trim(), student_ids: groupStudents }; const group = editingGroupId ? await updateTeacherGroup(apiUrl, editingGroupId, payload) : await createTeacherGroup(apiUrl, payload); setGroups((items) => editingGroupId ? items.map((item) => item.id === group.id ? group : item) : [group, ...items]); resetGroupForm(); const dashboard = await getTeacherDashboard(apiUrl); setData(dashboard); setLabs(dashboard.labs); setReviews(dashboard.reviews); setGroups(dashboard.groups); setNotice(group.name + " has been saved."); } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to save the group."); } }
  async function removeGroup(group: ApiStudentGroup) { if (!window.confirm("Remove " + group.name + "?")) return; try { const result = await deleteTeacherGroup(apiUrl, group.id); setGroups((items) => items.filter((item) => item.id !== group.id)); const dashboard = await getTeacherDashboard(apiUrl); setData(dashboard); setLabs(dashboard.labs); setReviews(dashboard.reviews); setGroups(dashboard.groups); setNotice(result.message); } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to remove the group."); } }
  ...
  const logPanel = processLog ? <Panel eyebrow="Telemetry report" title="Attack chain reconstruction"><pre className="max-h-80 overflow-auto whitespace-pre-wrap bg-ink p-4 text-xs leading-5 text-white">{processLog}</pre></Panel> : null;
  const groupPanel = <Panel eyebrow="Student groups" title={editingGroupId ? "Edit group" : "Create group"}><form onSubmit={saveGroup} className="space-y-4 p-4"><label className="text-sm font-bold text-ink/72">Group name<input value={groupName} onChange={(event) => setGroupName(event.target.value)} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" placeholder="Blue team cohort" /></label><div><p className="text-sm font-black text-ink">Students</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{data.students.map((student) => <label key={student.id} className={clsx("flex min-h-11 items-center justify-between gap-3 rounded-md border px-3 text-sm font-bold", groupStudents.includes(student.id) ? "border-fern bg-mint/10 text-fern" : "border-ink/10 text-ink/65")}><span>{student.name}<span className="ml-2 text-xs font-semibold text-ink/45">{student.active_labs} labs</span></span><input type="checkbox" checked={groupStudents.includes(student.id)} onChange={() => toggleGroupStudent(student.id)} className="h-4 w-4 accent-[#2f6f5f]" /></label>)}</div></div><div className="flex flex-wrap justify-between gap-2 border-t border-ink/10 pt-4">{editingGroupId ? <button type="button" onClick={resetGroupForm} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-ink/12 px-3 text-sm font-bold text-ink/65 hover:border-fern/50"><X size={16} />Cancel</button> : <span />}<button disabled={!groupName.trim()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-canopy px-3 text-sm font-bold text-white disabled:opacity-50"><Save size={16} />Save group</button></div></form><div className="divide-y divide-ink/10">{groups.length ? groups.map((group) => <div key={group.id} className="flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="text-sm font-black text-ink">{group.name}</p><p className="mt-1 text-xs text-ink/54">{group.student_count} students - {group.lab_count} assigned labs</p></div><div className="flex gap-2"><button type="button" onClick={() => editGroup(group)} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-ink/12 px-3 text-xs font-bold text-ink/65 hover:border-fern/50"><Edit3 size={15} />Edit</button><button type="button" onClick={() => removeGroup(group)} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-clay/25 px-3 text-xs font-bold text-clay hover:bg-clay/10"><Trash2 size={15} />Remove</button></div></div>) : <p className="p-6 text-center text-sm text-ink/54">No groups yet.</p>}</div></Panel>;
  if (view === "Class labs") return <div className="space-y-5"><Notice text={notice} />{logPanel}<Panel eyebrow="Class labs" title="Author and publish labs"><LabForm machines={data.machines} students={data.students} groups={groups} value={newLab} onChange={setNewLab} onSubmit={createLab} submitLabel="Create lab" saving={saving} /><LabList labs={labs} machines={data.machines} students={data.students} groups={groups} editable onSave={saveLab} onDelete={removeLab} onStart={startManagedLab} onStop={stopManagedLab} onDownloadVpn={downloadManagedVpn} onReport={loadManagedReport} /></Panel></div>;
  if (view === "Students") return <div><Notice text={notice} /><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.95fr)]"><Panel eyebrow="Learners" title="Class progress"><div className="divide-y divide-ink/10">{data.students.map((student) => <div key={student.id} className="p-4"><div className="flex justify-between gap-3"><div><p className="text-sm font-black text-ink">{student.name}</p><p className="mt-1 text-xs text-ink/54">{student.cohort} - {student.active_labs} assigned - {student.running_labs} running</p></div><p className="text-sm font-bold text-fern">{student.progress}%</p></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-cloud"><div className="h-full bg-fern" style={{ width: student.progress + "%" }} /></div></div>)}</div></Panel>{groupPanel}<Panel eyebrow="Review queue" title="Student activity"><div className="divide-y divide-ink/10">{reviews.length ? reviews.map((item) => <div key={item.id} className="p-4"><p className="text-sm font-black text-ink">{item.student}</p><p className="mt-1 text-xs text-ink/54">{item.lab}</p><div className="mt-3 flex items-center justify-between gap-3"><span className="text-xs font-bold text-fern">{item.state}</span><Button icon={ClipboardCheck} onClick={() => review(item.id)}>Review</Button></div></div>) : <p className="p-6 text-center text-sm text-ink/54">Review queue is clear.</p>}</div></Panel></div></div>;
  if (view === "Machine library") return library;
  return <div><Notice text={notice} />{logPanel ? <div className="mb-5">{logPanel}</div> : null}<div className="grid gap-4 md:grid-cols-3">{[["Published labs", String(labs.filter((lab) => lab.status !== "locked").length), Layers3], ["Active students", String(data.students.length), Users], ["Running sessions", String(data.metrics?.running_sessions ?? 0), Activity]].map(([label, value, Icon]) => { const Metric = Icon as typeof Activity; return <div key={label as string} className="rounded-lg border border-ink/10 bg-white p-4 shadow-sm"><div className="flex justify-between"><div><p className="text-sm font-semibold text-ink/55">{label as string}</p><p className="mt-2 text-3xl font-black text-ink">{value as string}</p></div><span className="grid h-10 w-10 place-items-center rounded-md bg-mint/20 text-canopy"><Metric size={20} /></span></div></div>; })}</div><div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]"><Panel eyebrow="Class labs" title="Your catalogue"><LabList labs={labs.slice(0, 3)} machines={data.machines} students={data.students} groups={groups} editable onSave={saveLab} onDelete={removeLab} onStart={startManagedLab} onStop={stopManagedLab} onDownloadVpn={downloadManagedVpn} onReport={loadManagedReport} /></Panel><Panel eyebrow="Review queue" title="Needs attention"><div className="divide-y divide-ink/10">{reviews.slice(0, 3).map((item) => <div key={item.id} className="p-4"><p className="text-sm font-black text-ink">{item.student}</p><p className="mt-1 text-xs text-ink/54">{item.lab}</p></div>)}</div></Panel></div></div>;
}

function MachineCard({ machine, onEdit, onRefresh }: { machine: ApiMachine; onEdit?: (machine: ApiMachine) => void; onRefresh?: (machine: ApiMachine) => void }) { const detailCount = machine.ports.length + machine.volumes.length + Object.keys(machine.environment).length + machine.cap_add.length; return <div className="p-4"><div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center rounded-md bg-mint/20 text-canopy"><ServerCog size={19} /></span><div className="flex gap-2"><span className="rounded-md bg-cloud px-2 py-1 text-xs font-bold text-ink/55">{machine.source_type}</span><span className="rounded-md bg-cloud px-2 py-1 text-xs font-bold text-ink/55">{machine.os_type}</span></div></div><p className="mt-4 text-sm font-black text-ink">{machine.name}</p><p className="mt-1 text-xs leading-5 text-ink/54">{machine.description || "No description provided."}</p><p className="mt-3 truncate text-xs font-semibold text-fern">{machine.imageUrl}</p><p className="mt-2 text-xs font-semibold text-ink/45">{machine.restart_policy} - {machine.memory_limit} RAM - {machine.cpu_limit} CPU{machine.privileged ? " - privileged" : ""}{detailCount ? " - " + detailCount + " runtime options" : ""}{machine.import_version ? " - import v" + machine.import_version : ""}</p><div className="mt-3 flex flex-wrap gap-2">{onEdit ? <button type="button" onClick={() => onEdit(machine)} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-ink/12 px-3 text-xs font-bold text-ink/65 hover:border-fern/50"><Edit3 size={15} />Edit</button> : null}{onRefresh && machine.repository_url ? <button type="button" onClick={() => onRefresh(machine)} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-fern/25 px-3 text-xs font-bold text-fern hover:bg-mint/10"><Download size={15} />Refresh from GitHub</button> : null}</div></div>; }
...
function keyValues(value: string) {
  return Object.fromEntries(lines(value).map((item) => {
    ...
    return splitAt === -1 ? [item, ""] : [item.slice(0, splitAt).trim(), item.slice(splitAt + 1).trim()];
  }).filter(([key]) => key));
}
...
function machineDraftFrom(machine: ApiMachine): MachineDraft {
  const pairs = (items: Record<string, string>) => Object.entries(items).map(([key, value]) => key + "=" + value).join("\n");
  return {
    name: machine.name,
...
function MachineForm({ value, onChange, onSubmit, submitLabel, onCancel }: { value: MachineDraft; onChange: (value: MachineDraft) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; submitLabel: string; onCancel?: () => void }) {
  return <form onSubmit={onSubmit} className="space-y-5 border-b border-ink/10 p-4">
    <div className="grid gap-3 lg:grid-cols-[1fr_180px_1fr]">
      <label className="text-sm font-bold text-ink/72">Machine name<input value={value.name} onChange={(event) => onChange({ ...value, name: event.target.value })} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" placeholder="DVWA target" /></label>
    ...
    <div className="grid gap-3 lg:grid-cols-3">
      ...
      <label className="text-sm font-bold text-ink/72">Working directory<input value={value.working_dir} onChange={(event) => onChange({ ...value, working_dir: event.target.value })} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" placeholder="/app" /></label>
      <label className="text-sm font-bold text-ink/72">Run as user<input value={value.run_as} onChange={(event) => onChange({ ...value, run_as: event.target.value })} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" placeholder="1000:1000" /></label>
      <label className="text-sm font-bold text-ink/72">Command<input value={value.command} onChange={(event) => onChange({ ...value, command: event.target.value })} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" /></label>
      <label className="text-sm font-bold text-ink/72">Entrypoint<input value={value.entrypoint} onChange={(event) => onChange({ ...value, entrypoint: event.target.value })} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" /></label>
      <label className="text-sm font-bold text-ink/72">Restart policy<select value={value.restart_policy} onChange={(event) => onChange({ ...value, restart_policy: event.target.value as MachineInput["restart_policy"] })} className="mt-2 min-h-10 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern"><option value="unless-stopped">unless-stopped</option><option value="no">no</option><option value="always">always</option><option value="on-failure">on-failure</option></select></label>
    </div>
    <div className="grid gap-3 lg:grid-cols-3">
      {[
        ["Ports", "ports", "8080:80"],
      ...
      ].map(([label, key, placeholder]) => <label key={key} className="text-sm font-bold text-ink/72">{label}<textarea value={value[key as keyof MachineDraft] as string} onChange={(event) => onChange({ ...value, [key]: event.target.value })} className="mt-2 min-h-24 w-full resize-y rounded-md border border-ink/15 px-3 py-2 text-sm outline-none focus:border-fern" placeholder={placeholder + "\nOne per line"} /></label>)}
    </div>
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-ink/10 pt-4">
      <div className="flex flex-wrap gap-4">
        <label className="inline-flex min-h-10 items-center gap-2 text-sm font-bold text-ink/65"><input type="checkbox" checked={value.tty} onChange={(event) => onChange({ ...value, tty: event.target.checked })} className="h-4 w-4 accent-[#2f6f5f]" />TTY</label>
        <label className="inline-flex min-h-10 items-center gap-2 text-sm font-bold text-ink/65"><input type="checkbox" checked={value.stdin_open} onChange={(event) => onChange({ ...value, stdin_open: event.target.checked })} className="h-4 w-4 accent-[#2f6f5f]" />Open stdin</label>
        <label className="inline-flex min-h-10 items-center gap-2 text-sm font-bold text-ink/65"><input type="checkbox" checked={value.privileged} onChange={(event) => onChange({ ...value, privileged: event.target.checked })} className="h-4 w-4 accent-[#2f6f5f]" />Privileged</label>
      </div>
      <div className="flex flex-wrap gap-2">{onCancel ? <button type="button" onClick={onCancel} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-ink/12 px-3 text-sm font-bold text-ink/65 hover:border-fern/50"><X size={16} />Cancel</button> : null}<button disabled={!value.name.trim() || !value.image_url.trim()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-canopy px-3 text-sm font-bold text-white disabled:opacity-50"><Plus size={16} />{submitLabel}</button></div>
    </div>
  </form>;
}

function AdminPortal({ apiUrl, view }: { apiUrl: string; view: string }) {
  const [data, setData] = useState<AdminDashboard | null>(null); const [labs, setLabs] = useState<ApiLab[]>([]); const [machines, setMachines] = useState<ApiMachine[]>([]); const [users, setUsers] = useState<AdminDashboard["users"]>([]); const [settings, setSettings] = useState<AdminDashboard["settings"]>([]); const [notice, setNotice] = useState(""); const [processLog, setProcessLog] = useState(""); const [machineDraft, setMachineDraft] = useState<MachineDraft>(emptyMachineDraft); const [editingMachineId, setEditingMachineId] = useState<string | null>(null); const [githubImport, setGitHubImport] = useState<GitHubMachineImportInput>({ repository_url: "", ref: "main", machine_path: "" }); const [githubFolders, setGitHubFolders] = useState<GitHubMachineFolder[]>([]); const [inspecting, setInspecting] = useState(false); const [importing, setImporting] = useState(false);
  useEffect(() => { getAdminDashboard(apiUrl).then((dashboard) => { setData(dashboard); setLabs(dashboard.labs); setMachines(dashboard.machines); setUsers(dashboard.users); setSettings(dashboard.settings); }).catch((error) => setNotice(error instanceof Error ? error.message : "Unable to load platform controls.")); }, [apiUrl]);
  async function saveMachine(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { const payload = machinePayload(machineDraft); const machine = editingMachineId ? await updateAdminMachine(apiUrl, editingMachineId, payload) : await createAdminMachine(apiUrl, payload); setMachines((items) => editingMachineId ? items.map((item) => item.id === machine.id ? machine : item) : [machine, ...items]); setMachineDraft(emptyMachineDraft); setEditingMachineId(null); setNotice(machine.name + (editingMachineId ? " has been updated." : " has been approved.")); } catch (error) { setNotice(error instanceof Error ? error.message : "Enter a valid machine definition."); } }
  async function importMachine(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setImporting(true); setNotice("Downloading and validating the GitHub machine..."); try { const machine = await importGitHubMachine(apiUrl, { repository_url: githubImport.repository_url.trim(), ref: githubImport.ref.trim(), machine_path: githubImport.machine_path.trim() }); setMachines((items) => [machine, ...items]); setGitHubImport({ repository_url: "", ref: "main", machine_path: "" }); setNotice(machine.name + " was imported and approved with its detections and attachments."); } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to import the GitHub machine."); } finally { setImporting(false); } }
  async function inspectRepository() { setInspecting(true); setGitHubFolders([]); setGitHubImport((value) => ({ ...value, machine_path: "" })); setNotice("Inspecting the repository for machine folders..."); try { const result = await discoverGitHubMachines(apiUrl, { repository_url: githubImport.repository_url.trim(), ref: githubImport.ref.trim() }); setGitHubFolders(result.machines); setGitHubImport((value) => ({ ...value, machine_path: result.machines[0]?.path ?? "" })); setNotice("Found " + result.machines.length + " importable machine" + (result.machines.length === 1 ? "." : "s. Choose one and import it.")); } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to inspect the GitHub repository."); } finally { setInspecting(false); } }
  function editMachine(machine: ApiMachine) { setEditingMachineId(machine.id); setMachineDraft(machineDraftFrom(machine)); setNotice(""); }
  function cancelMachineEdit() { setEditingMachineId(null); setMachineDraft(emptyMachineDraft); }
  async function refreshMachine(machine: ApiMachine) { if (!window.confirm("Refresh " + machine.name + " from " + machine.repository_ref + "?")) return; setNotice("Refreshing " + machine.name + "..."); try { const refreshed = await refreshGitHubMachine(apiUrl, machine.id); setMachines((items) => items.map((item) => item.id === refreshed.id ? refreshed : item)); setNotice(refreshed.message); } catch (error) { setNotice(error instanceof Error ? error.message : "Unable to refresh the machine."); } }
  ...
  if (view === "Machine fleet") return <div className="space-y-5"><Notice text={notice} /><Panel eyebrow="Repository import" title="Import a machine from GitHub"><form onSubmit={importMachine} className="space-y-4 p-4"><p className="text-sm leading-6 text-ink/58">Enter a public GitHub repository and ref, then let Mayajal find every folder containing <strong>machine.json</strong> and <strong>Dockerfile</strong>. Choose one machine to validate and import.</p><div className="grid gap-3 lg:grid-cols-[minmax(280px,1.5fr)_minmax(130px,0.5fr)_auto] lg:items-end"><label className="text-sm font-bold text-ink/72">Repository URL<input type="url" required value={githubImport.repository_url} onChange={(event) => { setGitHubImport({ ...githubImport, repository_url: event.target.value, machine_path: "" }); setGitHubFolders([]); }} className="mt-2 min-h-11 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" placeholder="https://github.com/owner/repository" /></label><label className="text-sm font-bold text-ink/72">Git ref<input required value={githubImport.ref} onChange={(event) => { setGitHubImport({ ...githubImport, ref: event.target.value, machine_path: "" }); setGitHubFolders([]); }} className="mt-2 min-h-11 w-full rounded-md border border-ink/15 px-3 text-sm outline-none focus:border-fern" placeholder="main" /></label><button type="button" onClick={inspectRepository} disabled={inspecting || !githubImport.repository_url.trim() || !githubImport.ref.trim()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-fern/30 px-4 text-sm font-bold text-fern hover:bg-mint/10 disabled:opacity-50">{inspecting ? <LoaderCircle size={16} className="animate-spin" /> : <Download size={16} />}{inspecting ? "Finding machines" : "Find machines"}</button></div>{githubFolders.length ? <label className="block text-sm font-bold text-ink/72">Machine to import<select required value={githubImport.machine_path} onChange={(event) => setGitHubImport({ ...githubImport, machine_path: event.target.value })} className="mt-2 min-h-12 w-full rounded-md border border-ink/15 bg-white px-3 text-sm outline-none focus:border-fern">{githubFolders.map((folder) => <option key={folder.path} value={folder.path}>{folder.name} — {folder.os_type} — {folder.path}</option>)}</select>{githubImport.machine_path ? <span className="mt-2 block text-xs font-medium leading-5 text-ink/50">{githubFolders.find((folder) => folder.path === githubImport.machine_path)?.description}</span> : null}</label> : <p className="rounded-md bg-cloud px-3 py-3 text-sm font-semibold text-ink/54">No repository inspected yet. Select “Find machines” to populate the machine-folder list.</p>}<div className="flex justify-end"><button disabled={importing || !githubImport.machine_path} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-canopy px-4 text-sm font-bold text-white disabled:opacity-50">{importing ? <LoaderCircle size={16} className="animate-spin" /> : <Plus size={16} />}{importing ? "Importing and validating" : "Import selected machine"}</button></div></form></Panel><Panel eyebrow="Machine fleet" title={editingMachineId ? "Edit machine image" : "Approve a machine image"}><MachineForm value={machineDraft} onChange={setMachineDraft} onSubmit={saveMachine} submitLabel={editingMachineId ? "Save machine" : "Add machine"} onCancel={editingMachineId ? cancelMachineEdit : undefined} /><div className="grid divide-y divide-ink/10 sm:grid-cols-2 sm:divide-x sm:divide-y-0">{machines.map((machine) => <MachineCard key={machine.id} machine={machine} onEdit={editMachine} onRefresh={refreshMachine} />)}</div></Panel></div>;
  ...
  const logPanel = processLog ? <Panel eyebrow="Container output" title="Docker Compose stream"><pre className="max-h-80 overflow-auto whitespace-pre-wrap bg-ink p-4 text-xs leading-5 text-white">{processLog}</pre></Panel> : null;
  if (view === "Lab catalogue") return <div className="space-y-5"><Notice text={notice} />{logPanel}<Panel eyebrow="Lab catalogue" title="Platform lab inventory"><LabList labs={labs} machines={machines} onStart={startManagedLab} onStop={stopManagedLab} onDownloadVpn={downloadManagedVpn} onReport={loadManagedReport} /></Panel></div>;
  return <div><Notice text={notice} />{logPanel ? <div className="mb-5">{logPanel}</div> : null}<div className="grid gap-4 md:grid-cols-4">{[["Running sessions", String(data.metrics?.running_sessions ?? 0), Activity], ["Students", String(data.metrics?.students ?? users.filter((user) => user.role === "student").length), Users], ["Labs", String(data.metrics?.labs ?? labs.length), Layers3], ["Approved images", String(machines.length), ServerCog]].map(([label, value, Icon]) => { const Metric = Icon as typeof Activity; return <div key={label as string} className="rounded-lg border border-ink/10 bg-white p-4 shadow-sm"><div className="flex justify-between"><div><p className="text-sm font-semibold text-ink/55">{label as string}</p><p className="mt-2 text-3xl font-black text-ink">{value as string}</p></div><span className="grid h-10 w-10 place-items-center rounded-md bg-mint/20 text-canopy"><Metric size={20} /></span></div></div>; })}</div><div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]"><Panel eyebrow="Running sessions" title="Student lab activity"><div className="divide-y divide-ink/10">{data.running_sessions.length ? data.running_sessions.map((session) => <div key={session.id} className="grid gap-2 p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-center"><div><p className="text-sm font-black text-ink">{session.student}</p><p className="mt-1 text-xs text-ink/54">{session.lab}</p></div><p className="text-xs font-semibold text-ink/54">{new Date(session.started_at).toLocaleString()}</p><span className="rounded-full bg-mint/20 px-2.5 py-1 text-xs font-bold text-fern">{session.status}</span></div>) : <p className="p-6 text-center text-sm text-ink/54">No running sessions.</p>}</div></Panel><Panel eyebrow="System health" title="Service status"><div className="divide-y divide-ink/10">{data.health.map((item) => <div key={item.name} className="flex items-center justify-between p-4"><span className="text-sm font-black text-ink">{item.name}</span><span className="inline-flex items-center gap-2 text-sm font-bold text-fern"><Check size={16} />{item.value}</span></div>)}</div></Panel></div></div>;
}
